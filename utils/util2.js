...
function parseStory(html, isDecode) {
    ...
    var questionArr = $(html).tag('div').attr('class', 'question').match();
        ...
        if (questionArr) {
            for (var i = 0, len = questionArr.length; i < len; i++) {
                $story = $(questionArr[i]);
                stories.push({
                    img: getArrayContent($story.match()).jhe_ma('img', 'src'),
                    content: parseStoryContent($story, isDecode)
                });
            }
        }
    ...
    //     var $story = $(html);
    //     stories.push({
    //         title: getArrayContent($story.tag('h3').match()),
    //         content: parseStoryContent($story, isDecode)
    //     });
...
function parseStoryContent( $story, isDecode ) {
  ...
  var p, strong, img, blockquote, em, h1, h2, h3, h4, h5, h6;
  if( ps ) {
    for( var i = 0, len = ps.length;i < len;i++ ) {
      ...
      h1 = getArrayContent( p.jhe_om( 'h1' ) );
      h2 = getArrayContent( p.jhe_om( 'h2' ) );
      h3 = getArrayContent( p.jhe_om( 'h3' ) );
      h4 = getArrayContent( p.jhe_om( 'h4' ) );
      h5 = getArrayContent( p.jhe_om( 'h5' ) );
      h6 = getArrayContent( p.jhe_om( 'h6' ) );
      ...
      else if (isOnly(p, h1)) { //获取引用块 <p><h1>...</h1></p>
        h1 = decodeHtml(h1, isDecode);
        if (!h1.isEmpty())
          content.push({ type: 'h1', value: h1 });
      }
      else if (isOnly(p, h2)) { //获取引用块 <p><h1>...</h1></p>
        h2 = decodeHtml(h2, isDecode);
        if (!h2.isEmpty())
          content.push({ type: 'h2', value: h2 });
      }
      else if (isOnly(p, h3)) { //获取引用块 <p><h1>...</h1></p>
        h3 = decodeHtml(h3, isDecode);
        if (!h3.isEmpty())
          content.push({ type: 'h3', value: h3 });
      }
      else if (isOnly(p, h4)) { //获取引用块 <p><h1>...</h1></p>
        h4 = decodeHtml(h4, isDecode);
        if (!h4.isEmpty())
          content.push({ type: 'h4', value: h4 });
      }
      else if (isOnly(p, h5)) { //获取引用块 <p><h1>...</h1></p>
        h5 = decodeHtml(h5, isDecode);
        if (!h5.isEmpty())
          content.push({ type: 'h5', value: h5 });
      }
      else if (isOnly(p, h6)) { //获取引用块 <p><h1>...</h1></p>
        h6 = decodeHtml(h6, isDecode);
        if (!h6.isEmpty())
          content.push({ type: 'h6', value: h6 });
      }
